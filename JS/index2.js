...
function isValidRotate(matrix, cellRow, cellCol) {
  //перебор, который прохоит по длине матрицы(фигуры), проверяя строки и проходит по длине матрице, проверяя колонки
  for (let row = 0; row < matrix.length; row++) {
    for (let col = 0; col < matrix[row].length; col++) {
      //Перебор будет проходить и дальше, если
      if (
        //элементы матрицы(строки и колонки) и
        matrix[row][col] &&
        //ячейки колонок меньше нулевой позиции (ячейки находятся в массиве. Массив начинается с позиции 0. Тобишь, если колонки находятся перед нулевой позицией - это плохо)
        (cellCol + col < 0 ||
          //или ячейки колонки нахо-ся за пределами массива(поля)
          cellCol + col >= arr[0].length ||
          //или ячейки строк нах-ся за пределами массива(поля)
          cellRow + row >= arr.length ||
          //или строки и колонки вовсе друг на руга налезли. Да, это вот так проверяется.
